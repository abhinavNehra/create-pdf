...
import html2canvas from 'html2canvas'; // Function to create the pdf
...
async function createPdf(id) {
  ...
  console.log('id --------', document.querySelector(id)); // Converting all the element into canvas and storing them in an array
  ...
  pdf.internal.scaleFactor = 2.25; // aspect height of the canvas

  height = canvas.height * width / canvas.width; // fill the background of the pdf with the background color of the pdf

  pdf.setFillColor(240, 242, 245); // adding the border of the pdf

  pdf.rect(0, 0, pdf.internal.pageSize.width, pdf.internal.pageSize.height, 'F'); // Adding the canvas into the pdf
  ...
  pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, position, width, height, undefined, 'FAST'); // adding border in the pdf

  pdf.rect(0, 0, pdf.internal.pageSize.width, pdf.internal.pageSize.height, 'S'); // If the aspect height of the canvas is greater than 297mm then portion after 297mm in first page get
  ...
  pages = Math.ceil((height - pageHeight) / pageHeight); // printed the remaining portion of the canvas on the pdf

  for (let i = 1; i <= pages; i++) {
    // adding new page
    pdf.addPage(); // set background color

    pdf.setFillColor(240, 242, 245); // adding border of pdf

    pdf.rect(0, 0, pdf.internal.pageSize.width, pdf.internal.pageSize.height, 'F'); // It is the important part of the this function
    ...
    position = -1 * (pageHeight * i) + 17 * i; // adding the content to the pdf

    pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, position, width, height); // adding border
